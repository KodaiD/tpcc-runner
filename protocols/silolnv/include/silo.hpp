...
#include "protocols/silolnv/include/readwriteset.hpp"
#include "protocols/silolnv/include/tidword.hpp"
...
public:
    ...
    SiloLNV(TxID txid, uint32_t epoch) : txid(txid), starting_epoch(epoch) {
        LOG_INFO("START Tx, e: %u", starting_epoch);
    }

    ~SiloLNV() { GarbageCollector::remove(starting_epoch); }
    ...
    Rec* insert(TableID table_id, Key key) {
        ...
        if (rw_iter == rw_table.end()) {
            ...
            new_val->tidword.latest = 1;  // exists in index
            new_val->tidword.absent = 1;  // cannot be seen by others
            typename Index::Result res = idx.insert(table_id, key, new_val, old_val, nm);

            if (res == Index::Result::ALREADY_INSERTED) {
    ...
    bool precommit() {
        ...
        commit_tw.epoch = load_acquire(EpochManager<SiloLNV<Index>>::get_global_epoch());
